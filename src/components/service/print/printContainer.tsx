...
import { getId } from "../../../services/ordemVendaService";
...
const PrintContainer = ({ordemVendaId} : PrintContainerProps) => {
  ...
  const { isPending, data } = useQuery({
    queryKey: ["prestacaoById", ordemVendaId],
    queryFn: () => getId(ordemVendaId || ""),
  });
    ...
    return (
      <section className="flex flex-col gap-5 w-full items-center">
        ...
        <section className="block w-[60%] p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 overflow-x-auto">
          {data ? (
            <ComponentPrint ref={componentRef} ordemVenda={data} />
          ) : (
            <></>
          )}
        </section>
